...
class VerificationService {
    ...
    async validateAndUpdateToken(token) {
        let verification = await Verification.findOne({VerificationToken: token, IsExpired: false });
        if(!verification){
            return null;
        } else {
            let userId = verification.UserId;

            await Verification.updateOne({VerificationToken: token}, {IsExpired: true});
